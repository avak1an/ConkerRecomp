...
static void dsp_dma_run(DSPDMAState *s)
{
    ...
    while (!(s->next_block & NODE_POINTER_EOL)) {
        ...
        if (transfer_size > s->scratch_capacity) {
            uint8_t *larger = realloc(s->scratch_buffer, transfer_size);
            if (!larger) { fputs("DSP DMA allocation failed\n", stderr); abort(); }
            s->scratch_buffer = larger;
            s->scratch_capacity = transfer_size;
        }
        uint8_t *scratch_buf = s->scratch_buffer;
