...
/* The monitor/output stage runs once every APU_MONITOR_FRAME_DIV VP frames,
 * so one monitor frame represents exactly APU_MONITOR_SAMPLES of emulated
 * audio.  The host output stage must hand the device that many samples and
 * no more: it is the device draining the submission that paces the frame
 * thread, so submitting a longer buffer than the VP actually produced slows
 * the whole pipeline by the ratio between the two. */
#define APU_MONITOR_FRAME_DIV 8
#define APU_MONITOR_SAMPLES   (APU_MONITOR_FRAME_DIV * NUM_SAMPLES_PER_FRAME)
