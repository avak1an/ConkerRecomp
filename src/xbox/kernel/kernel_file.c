...
/* Create every missing directory along `path`, up to `upto` characters.
 *
 * Z: is the title's own cache partition, and Conker treats it as its to shape:
 * it queries Z:\texture\frontend\tips, is told the path is absent, and then
 * asks to create it in one call. CreateDirectoryW only ever creates the leaf,
 * so that came back ERROR_PATH_NOT_FOUND, the cached-texture write failed, and
 * the title dropped into its fatal-error handler.
 *
 * Pass the full length for a directory, or the offset of the last separator to
 * build only a file's parents. Intermediate failures are ignored on purpose --
 * the only result that matters is whether the CreateFileW that follows works.
 */
static void create_dir_tree(const WCHAR *path, size_t upto)
{
    WCHAR buf[MAX_PATH];
    size_t i;

    if (upto == 0 || upto >= MAX_PATH)
        return;
    memcpy(buf, path, upto * sizeof(WCHAR));
    buf[upto] = L'\0';

    /* Never try to create the drive itself. */
    i = (buf[0] != L'\0' && buf[1] == L':') ? 2u : 0u;
...
/* Build the directories leading to a file, leaving the file itself alone. */
static void create_parent_dirs(const WCHAR *path)
{
    size_t i, last = 0;
    ...
    if (last)
        create_dir_tree(path, last);
}

static int disposition_creates(ULONG d)
{
    return d == XBOX_FILE_SUPERSEDE || d == XBOX_FILE_CREATE ||
           d == XBOX_FILE_OPEN_IF || d == XBOX_FILE_OVERWRITE_IF;
}

/* The handle of the title's video stream, and whether reads on it should be
 * answered as end-of-stream (CONKER_XMV_EOF=1). */
static HANDLE g_video_handle;

void xbox_file_note_video_handle(HANDLE h)
{
    g_video_handle = h;
    fprintf(stderr, "[INFO FILE] video stream handle=%p\n", h);
}

int xbox_file_is_video_handle(HANDLE h)
{
    return h != NULL && h == g_video_handle;
}
    ...
    ULONG CreateDisposition, ULONG CreateOptions)
{
    ...
    /* Some Xbox callers open a volume root without DIRECTORY_FILE.  Win32
       still requires FILE_FLAG_BACKUP_SEMANTICS for an existing directory. */
    existing_attrs = GetFileAttributesW(win_path);
    opening_directory = ((CreateOptions & XBOX_FILE_DIRECTORY_FILE) != 0) ||
                        (existing_attrs != INVALID_FILE_ATTRIBUTES &&
                         (existing_attrs & FILE_ATTRIBUTE_DIRECTORY) != 0);

    if (opening_directory) {
        if (disposition_creates(CreateDisposition))
            create_dir_tree(win_path, wcslen(win_path));
        /* A host directory represents an Xbox volume root.  Unlike a normal
         * Win32 directory handle, it must tolerate multiple concurrent opens
         * by the title's filesystem bootstrap regardless of requested share
         * flags. */
    ...
    *FileHandle = h;
    ...
    /* Remember the video stream's handle.
     *
     * The title opens BootIntro.xmv last, reads 2 MiB of a 23 MiB file, and
     * then loops in its frontend forever -- it is not blocked, it is waiting
     * for its own playback to report finished, and nothing here decodes a
     * frame to tell it so.  Knowing which handle that is lets the read path
     * answer end-of-stream, which is the cheapest way to find out whether
     * end-of-stream is what it is waiting for. */
    {
        size_t len = wcslen(win_path);
        if (len >= 4 && _wcsicmp(win_path + len - 4, L".xmv") == 0)
            xbox_file_note_video_handle(h);
    }
    ...
    PLARGE_INTEGER ByteOffset)
{
    ...
    /* Partition roots are represented by host directories.  Some titles
       probe them with a small raw-sector read; Win32 rejects ReadFile on a
       directory, whereas the Xbox device returns sector data.  Supply a
       deterministic blank sector/device read so this probe remains harmless. */
    if (GetLastError() == ERROR_INVALID_FUNCTION && Buffer && Length <= 4096) {
        memset(Buffer, 0, Length);
        IoStatusBlock->Status = STATUS_SUCCESS;
        IoStatusBlock->Information = Length;
        XBOX_TRACE(XBOX_LOG_FILE, "NtReadFile(directory-backed volume, len=%u) -> zero data", Length);
        return STATUS_SUCCESS;
    }
    ...
    PLARGE_INTEGER ByteOffset)
{
    ...
    /* Partition roots are directory-backed in the host filesystem.  Conker
       writes a small cache/volume probe to that synthetic device; retain the
       data nowhere, but report a complete device write as the Xbox kernel
       would. */
    if (GetLastError() == ERROR_INVALID_FUNCTION && Buffer && Length <= 4096) {
        IoStatusBlock->Status = STATUS_SUCCESS;
        IoStatusBlock->Information = Length;
        ...
        XBOX_TRACE(XBOX_LOG_FILE,
                   "NtWriteFile(directory-backed volume, len=%u) -> discarded", Length);
        return STATUS_SUCCESS;
    }
    ...
    PVOID FileInformation, ULONG Length, XBOX_FILE_INFORMATION_CLASS FileInformationClass)
{
    ...
    switch (FileInformationClass) {
        ...
        case 56:
            /* Do not turn a malformed arbitrary-size query into repeated
             * multi-megabyte writes to emulated memory. */
            IoStatusBlock->Status = STATUS_INVALID_PARAMETER;
            IoStatusBlock->Information = 0;
            XBOX_TRACE(XBOX_LOG_FILE,
                       "NtQueryInformationFile(class=56, len=%u) -> invalid parameter", Length);
            return STATUS_INVALID_PARAMETER;
        default:
    ...
    PVOID FsInformation, ULONG Length, XBOX_FS_INFORMATION_CLASS FsInformationClass)
{
    ...
    switch (FsInformationClass) {
        case XboxFileFsVolumeInformation: {
            /* Returned NOT_IMPLEMENTED before, 116 times in a three-minute
             * run, while the title was building its audio and package cache
             * under save\\Cache and reopening the same entries repeatedly.
             * A partition that cannot describe itself is not a useful answer
             * to give a title deciding whether its cache is valid. */
            PXBOX_FILE_FS_VOLUME_INFORMATION vol =
                (PXBOX_FILE_FS_VOLUME_INFORMATION)FsInformation;
            if (Length < sizeof(XBOX_FILE_FS_VOLUME_INFORMATION))
                return STATUS_INFO_LENGTH_MISMATCH;
            vol->VolumeCreationTime.QuadPart = 0;
            vol->VolumeSerialNumber = 0x584D4544u; /* stable across runs */
            vol->VolumeLabelLength = 0;
            vol->SupportsObjects = FALSE;
            vol->VolumeLabel[0] = '\0';
            IoStatusBlock->Status = STATUS_SUCCESS;
            IoStatusBlock->Information =
                sizeof(XBOX_FILE_FS_VOLUME_INFORMATION);
            return STATUS_SUCCESS;
        }
        case XboxFileFsAttributeInformation: {
            /* Asked for immediately after the volume class, and refused just
             * as often -- 116 times a run.  FATX is what the title is
             * actually running on, and its 42-character component limit is
             * the one thing here a caller can behave differently on. */
            PXBOX_FILE_FS_ATTRIBUTE_INFORMATION att =
                (PXBOX_FILE_FS_ATTRIBUTE_INFORMATION)FsInformation;
            static const char fs_name[] = "FATX";
            ULONG name_bytes = (ULONG)(sizeof(fs_name) - 1u);
            ULONG fixed = (ULONG)(sizeof(XBOX_FILE_FS_ATTRIBUTE_INFORMATION) -
                                  sizeof(att->FileSystemName));
            ULONG room;
            if (Length < fixed)
                return STATUS_INFO_LENGTH_MISMATCH;
            att->FileSystemAttributes = 0;
            att->MaximumComponentNameLength = 42;
            att->FileSystemNameLength = name_bytes;
            room = Length - fixed;
            if (room > name_bytes)
                room = name_bytes;
            memcpy(att->FileSystemName, fs_name, room);
            IoStatusBlock->Status = STATUS_SUCCESS;
            IoStatusBlock->Information = fixed + room;
            return room == name_bytes ? STATUS_SUCCESS
        ...
        case XboxFileFsSizeInformation: {
            ...
            if (GetDiskFreeSpaceExW(NULL, &free_bytes, &total_bytes, &total_free)) {
                ...
                /* Conker verifies a 16 KiB Xbox allocation unit at startup. */
