...
#include <string.h>
#include <ctype.h>
#include <dbghelp.h>   /* TEMPORARY: backtrace for the malformed-path probe */
...
static const path_rule s_rules[] = {
    ...
    /* Retail titles may probe the dashboard/HDD root before continuing.
       There is no separate dashboard volume in this host runtime, so expose
       the writable save root as a harmless directory-backed partition. */
    { "\\Device\\Harddisk0\\Partition0",     0, NULL,         NULL          },
...
translate:
    {
        ...
        if (sub_dir) {
            ...
            /* Z: is an Xbox cache volume.  Titles commonly open their
             * title-owned cache slot (for example Z:\\TitleName) with
             * FILE_OPEN, expecting the dashboard to have provisioned that
             * directory already.  On the host there is no dashboard, so
             * create a simple first-level cache slot on demand.  Do not
             * create arbitrary nested paths here: those may be files whose
             * parent is the cache root. */
            if (wcscmp(sub_wide, L"\\Cache") == 0 &&
                remainder_wide[0] != L'\0' &&
                wcschr(remainder_wide, L'\\') == NULL &&
                wcschr(remainder_wide, L'.') == NULL) {
                CreateDirectoryW(host_path_buf, NULL);
            }
        } else {
        ...
        /* TEMPORARY: a path carrying the executable's own file name in the
         * middle of it, or a formatted NULL, is malformed.  Name the guest
         * routine that built it -- recompiled functions appear by name in a
         * native backtrace. */
        if (strstr(xbox_path, ".xbe") != NULL ||
            strstr(xbox_path, "(null)") != NULL) {
            static unsigned traced;
            if (traced++ < 2u) {
                void *frames[18];
                USHORT n = CaptureStackBackTrace(0, 18, frames, NULL);
                HANDLE proc = GetCurrentProcess();
                char buf[sizeof(SYMBOL_INFO) + MAX_SYM_NAME] = {0};
                SYMBOL_INFO *sym = (SYMBOL_INFO *)buf;
                USHORT i;

                fprintf(stderr, "[PATHBUG] malformed path: %s\n", xbox_path);
                if (SymInitialize(proc, NULL, TRUE)) {
                    sym->SizeOfStruct = sizeof(*sym);
                    sym->MaxNameLen = MAX_SYM_NAME;
                    for (i = 0; i < n; ++i) {
                        DWORD64 disp = 0;
                        if (SymFromAddr(proc, (DWORD64)(uintptr_t)frames[i],
                                        &disp, sym))
                            fprintf(stderr, "[PATHBUG]   %s+0x%llX\n",
