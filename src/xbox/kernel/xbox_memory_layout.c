...
/*
 * The simulated CPU register state and ICALL diagnostics live in
 * recomp_globals.c.  Keep their definitions in that single translation
 * unit; the generated code accesses them through extern declarations in
 * recomp_types.h.
 */
...
BOOL xbox_MemoryLayoutInit(const void *xbe_data, size_t xbe_size)
{
     ...
     */
    {
		...
		fprintf(stderr, "[DEBUG] g_memory_offset = 0x%p\n",
        (void*)g_memory_offset);

		fprintf(stderr, "[DEBUG] 0x8001003C native = 0x%p\n",
        (void*)((uintptr_t)0x8001003C + g_memory_offset));
		...
		fprintf(stderr, "[DEBUG] VirtualAlloc returned %p\n", g_kernel_memory);
...
uint32_t xbox_HeapAlloc(uint32_t size, uint32_t alignment)
{
    ...
    /* The active runtime mapping is owned by xbox_memory.c.  This older
     * layout module is also compiled for metadata compatibility, but its
     * private g_memory_offset is never initialized in the normal runtime.
     * Share the active bump allocator and its mapping offset so a returned
     * Xbox VA is never accidentally used as a native host pointer. */
    result = xbox_heap_alloc(size, alignment);
    if (!result) {
        fprintf(stderr, "xbox_HeapAlloc: out of memory (requested %u)\n", size);
        return 0;
    }

    /* Zero-fill through the actual mapped Xbox memory view. */
    memset((void *)((uintptr_t)result + g_xbox_mem_offset), 0, size);

    g_heap_alloc_count++;
    fprintf(stderr, "  [HEAP] #%d: size=%u align=%u → 0x%08X..0x%08X (used %u/%u)\n",
            g_heap_alloc_count, size, alignment, result, result + size,
            result + size - XBOX_HEAP_BASE, XBOX_HEAP_SIZE);
    fflush(stderr);
...
{
    /* Return it to the allocator this module borrows from.  While the guest
     * heap could not free, discarding the address here cost nothing; now that
     * it can, every pool and contiguous block the title releases is address
     * space it gets to use again. */
    xbox_heap_free(xbox_va);
}
