...
KIRQL xbox_GetCurrentIrql(void)
{
...
/* The console's performance counter runs at a fixed 733,333,333 Hz, and
 * titles bake that in rather than dividing by what the kernel reports.
 *
 * Conker converts a counter delta to microseconds and scales by the constant
 * 1e-6 at 0x00649284. Returning the host's rate here -- 10 MHz on this machine
 * -- made every frame's tick delta about 73x too small, so the integer
 * conversion to microseconds truncated to **zero**: the title ran its update
 * 1.7 million times in 73 seconds with dt = 0 and nothing ever advanced.
 *
 * So present an Xbox-rate counter. The elapsed host ticks are rebased at first
 * use and scaled, splitting the division so a long session cannot overflow the
 * multiply (the remainder is below the host frequency, so rem * 733333333 stays
 * well inside 64 bits).
 */
#define XBOX_PERF_FREQUENCY 733333333LL

static LONGLONG g_perf_host_freq;
static LARGE_INTEGER g_perf_host_base;

LARGE_INTEGER __stdcall xbox_KeQueryPerformanceFrequency(void)
{
    LARGE_INTEGER freq;
    freq.QuadPart = XBOX_PERF_FREQUENCY;
    return freq;
}

/* The guest's rdtsc.
 *
 * Conker reads it twice a frame and converts the delta with `* 3 / 2200`, which
 * is exactly "divide by 733.33 MHz to get microseconds". The host TSC runs
 * several times faster, so returning it would make the title's clock run fast;
 * scale the elapsed host time to the console's CPU rate instead. Shares the
 * rebasing below so both clocks agree.
 */
uint64_t recomp_rdtsc(void)
{
    LARGE_INTEGER c = xbox_KeQueryPerformanceCounter();

    /* How the title actually sees this clock.
     *
     * The video controller reads it twice a frame and converts the delta with
     * * 3 / 2200, which is microseconds at 733,333,333 Hz.  Its playback clock
     * then runs about thirty times slow, so the question is whether the counter
     * advances at the rate it claims, and how big the deltas between successive
     * reads are -- a delta small enough to floor through the title's integer
     * conversion contributes nothing at all.  Opt-in: CONKER_RDTSC_TRACE=1. */
    { static int on = -1;
      static uint64_t calls, first_tick, last_tick, zero_deltas, tiny_deltas;
      static ULONGLONG first_wall;
      if (on < 0)
          on = getenv("CONKER_RDTSC_TRACE") != NULL;
      if (on) {
          uint64_t now = (uint64_t)c.QuadPart;
          uint64_t delta = now - last_tick;
          if (calls == 0) { first_tick = now; first_wall = GetTickCount64(); }
          else if (delta == 0) ++zero_deltas;
          else if (delta < 6600u) ++tiny_deltas;   /* floors to 0 through *3/2200 */
          last_tick = now;
          if (++calls % 100u == 0u && calls <= 4000u) {
              ULONGLONG wall = GetTickCount64() - first_wall;
              uint64_t ticks = now - first_tick;
              fprintf(stderr, "[RDTSC] calls=%llu wall=%llums ticks=%llu "
                      "rate=%.0fHz zero-deltas=%llu tiny=%llu\n",
                      (unsigned long long)calls, (unsigned long long)wall,
                      (unsigned long long)ticks,
                      wall ? (double)ticks * 1000.0 / (double)wall : 0.0,
                      (unsigned long long)zero_deltas,
                      (unsigned long long)tiny_deltas);
              fflush(stderr);
          }
      } }
...
{
    LARGE_INTEGER now, out;
    LONGLONG elapsed, whole, rem;

    if (g_perf_host_freq == 0) {
        LARGE_INTEGER f;
        QueryPerformanceFrequency(&f);
        g_perf_host_freq = f.QuadPart ? f.QuadPart : 1;
        QueryPerformanceCounter(&g_perf_host_base);
    }

    QueryPerformanceCounter(&now);
    elapsed = now.QuadPart - g_perf_host_base.QuadPart;
    whole = elapsed / g_perf_host_freq;
    rem   = elapsed % g_perf_host_freq;
    out.QuadPart = whole * XBOX_PERF_FREQUENCY
                 + (rem * XBOX_PERF_FREQUENCY) / g_perf_host_freq;
    return out;
}
    ...
    PVOID RegisterBase, ULONG Option, ULONG Param, PULONG Result)
{
    ...
    case AV_OPTION_QUERY_AVPACK:
        /* Video standard in the upper bits, AV pack in the low byte.
         *
         * Returning the bare pack looks harmless and is not.  Conker keys its
         * mode table at 0x005466B0 on two separate fields of this word: it
         * scans 185 twelve-byte rows for one whose first dword matches
         * (Result & 0xFF00), then rescans from there for (Result & 0xFF).
         * Every real row carries 0x0100 in the second byte -- the low half of
         * the NTSC-M standard code -- so a bare pack makes the first scan miss
         * every row, run off the end onto the FFFFFFFF terminator, and store
         * that as the device flags at [dev+0x1DDC].
         *
         * All ones there sets bits 21 and 24, which nothing on real hardware
         * sets, and the title reads that word in two places: the vblank
         * service routine branches on bit 24 into a path guarded by a port
         * 0x80C0 read, and sub_00539C60 tests (flags & 0x1200000) to decide
         * whether presentation is active.  With both bits set the routine
         * reports a flip permanently pending, and the XMV decoder -- which
         * refuses to resample its clock while one is -- never decodes a frame.
         *
         * We only need the lookup to hit a row; the flags actually stored come
         * from the title's own table, so this cannot invent a mode the game
         * does not ship.  NTSC-M with the HDTV pack selects the 640x480 row,
         * matching the device we create. */
        *Result = XC_VIDEO_STANDARD_NTSC_M | AV_PACK_HDTV;
        break;
