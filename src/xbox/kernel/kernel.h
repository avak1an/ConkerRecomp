...
#endif
/* The buffer a caller supplied is the wrong size for the class it asked for.
 * Its own guard: sharing one with a status the host headers already define
 * means the whole block is skipped and this never gets declared. */
...
/* FS_VOLUME_INFORMATION.  VolumeLabel is a variable-length trailing array;
 * the single character here is the terminator for an empty label, which is
 * what a title gets from an unnamed partition. */
...
/* FS_ATTRIBUTE_INFORMATION.  FileSystemName is a variable-length trailing
 * array, so a caller may supply a buffer shorter than the whole name. */
typedef struct _XBOX_FILE_FS_ATTRIBUTE_INFORMATION {
    ULONG FileSystemAttributes;
    LONG  MaximumComponentNameLength;
    ULONG FileSystemNameLength;
    CHAR  FileSystemName[1];
} XBOX_FILE_FS_ATTRIBUTE_INFORMATION, *PXBOX_FILE_FS_ATTRIBUTE_INFORMATION;
...
/* The guest VA window above the 64MB mapping, reserved by xbox_memory_init so
 * host allocations cannot land where the title's own reservations go.
 * NULL if the reservation failed. */
extern uint8_t *g_guest_window_base;
extern size_t   g_guest_window_size;
...
/* ---- EEPROM video standard codes ---- */
#define XC_VIDEO_STANDARD_NTSC_M  0x00400100
#define XC_VIDEO_STANDARD_NTSC_J  0x00400200
#define XC_VIDEO_STANDARD_PAL_I   0x00800300
