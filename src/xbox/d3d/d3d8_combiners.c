...
static CombinerCacheEntry *g_last_shader_entry;
 ...
 *
 * Output DWORD layout, as NV_PGRAPH_COMBINEOUTPUT packs it:
 ...
 *   [18]    CD blue-to-alpha; [19] AB blue-to-alpha (RGB path only).
 *
 * CD comes first, not AB.  The title's own words settle it without recourse to
 * a register table: the loading screen's stage 0 is 0x000020D0, which sets the
 * AB dot-product flag at bit 13 and has exactly one live destination nibble,
 * the 0xD at [7:4].  Read the other way round the hardware would compute a dot
 * product and send it to register 0 to be thrown away, and stage 2's
 * 0x000000C0 -- the whole point of that stage -- would likewise write nothing.
 * Reading AB at [7:4] makes both stages do work that reaches the screen.
 */
                                       ...
                                       NV2ACombinerState *state)
{
     ...
     *   [8]   general-combiner MUX uses MSB (1) vs LSB (0) of R0 alpha
     *   [12]  unique C0 per stage (1) vs shared (0)
     *   [16]  unique C1 per stage (1) vs shared (0)
     */
    state->mux_msb = (rs[D3DRS_PSCOMBINERCOUNT] >> 8) & 1u;
 ...
 *   3. Input struct (SV_POSITION, COLOR0, COLOR1, TEXCOORD0-4)
 ...
 *      d. Apply alpha test (fog blending is part of the final combiner)
 *      e. Return result
 * ================================================================ */
                               ...
                               const char *suffix, int stage_idx)
{
    ...
    if (input->alpha_rep) {
        ...
        /* In the scalar path channel-select zero means BLUE. */
        snprintf(swizzle, sizeof(swizzle), "%s", strcmp(suffix, ".a") == 0 ? ".b" : suffix);
    }
                                 ...
                                 char *buf, int bufsize)
{
    ...
    /* ---- Texture samplers ---- */
    EMIT("float bump_signed(float x) { x *= 255.0; return (x >= 128.0 ? x - 256.0 : x) / 127.0; }\n");
    ...
    EMIT("    float4 bump_matrix[4];\n");
    EMIT("    float4 bump_luminance[4];\n");
    EMIT("};\n\n");
    ...
    /* NV2A FOG combines constant RGB with the interpolated fog factor.
     * The packed fog color's alpha is not the factor. */
    ...
    for (i = 0; i < NV2A_MAX_TEXTURES; i++) {
        if (state->tex_mode[i] == NV2A_TEXMODE_NONE) {
        ...
        } else if (i > 0 && (state->tex_mode[i] == NV2A_TEXMODE_BUMPENV ||
                             state->tex_mode[i] == NV2A_TEXMODE_BUMPENV_LUM)) {
            unsigned input = state->bump_input[i];
            if (input >= (unsigned)i) return -1;
            /* NV097 BUMPENVMAP uses signed B/G displacement, not the
             * projective divide used by PROJECT2D. Matrix registers are
             * ordered 00,01,10,11 (column-major in the Xbox operation). */
            EMIT("    float2 dsdt%d = float2(bump_signed(r_t%u.b), bump_signed(r_t%u.g));\n", i, input, input);
            EMIT("    float2 bump_uv%d = input.tc%d.xy + float2(dot(bump_matrix[%d].xz, dsdt%d), dot(bump_matrix[%d].yw, dsdt%d));\n", i, i, i, i, i, i);
            EMIT("    float4 r_t%d = tex%d.Sample(samp%d, bump_uv%d);\n", i, i, i, i);
            if (state->tex_mode[i] == NV2A_TEXMODE_BUMPENV_LUM)
                EMIT("    r_t%d *= bump_luminance[%d].x * r_t%u.r + bump_luminance[%d].y;\n", i, i, input, i);
            ...
            EMIT("    float4 r_t%d = tex%d.Sample(samp%d, input.tc%d.xyz);\n",
                 i, i, i, i);
        } else if (state->tex_mode[i] == NV2A_TEXMODE_PROJECT2D) {
            EMIT("    float4 r_t%d = tex%d.Sample(samp%d, input.tc%d.xy / input.tc%d.w);\n",
                 i, i, i, i, i);
        } else {
    ...
    for (i = 0; i < state->num_stages; i++) {
        ...
        /* Sum or mux */
        if (rgb_out->mux_sum) {
            /* A set selector chooses CD; a clear selector chooses AB. */
            EMIT("        float3 sum_rgb = (%s) ? cd_rgb : ab_rgb;\n",
                 state->mux_msb ? "r_r0.a >= 0.5" : "((uint)(int)(r_r0.a * 255.0) & 1u) != 0u");
        } else {
        ...
        /*
         * Read both paths before committing either path's destinations.
         * Alpha can select a register's blue component from the same stage's
         * input state; an earlier RGB write must not change that read.
         */
        ...
        if (alpha_out->mux_sum) {
            EMIT("        float sum_a = (%s) ? cd_a : ab_a;\n",
                 state->mux_msb ? "r_r0.a >= 0.5" : "((uint)(int)(r_r0.a * 255.0) & 1u) != 0u");
        } else {
            EMIT("        float sum_a = ab_a + cd_a;\n");
        }

        /* Commit the RGB results, clamping the mapped values to [-1,1]. */
        if (rgb_out->ab_dst != NV2A_REG_ZERO) {
            ...
            if (rgb_out->ab_blue_to_alpha)
                EMIT("        %s.a = %s.b;\n", reg_name(rgb_out->ab_dst), reg_name(rgb_out->ab_dst));
        }
        if (rgb_out->cd_dst != NV2A_REG_ZERO) {
            ...
            if (rgb_out->cd_blue_to_alpha)
                EMIT("        %s.a = %s.b;\n", reg_name(rgb_out->cd_dst), reg_name(rgb_out->cd_dst));
        }
    ...
    /* Fog blending is explicitly encoded in the final combiner. Applying
     * another lerp here would fog that result twice. The fixed-function
     * pixel shader has its own fog path in d3d8_shaders.c. */
...
static ID3D11PixelShader *compile_combiner_shader(const NV2ACombinerState *state)
{
    ...
    hr = d3d8_compile_shader(hlsl, (SIZE_T)len, "ps_combiner",
                    "main", "ps_5_0",
...
{
    /* Write the generated source out once, when asked.  The combiner state
     * reaching here can look entirely reasonable and still produce a black
     * frame, and the shader is the only place that says which of the two it
     * is.  CONKER_RC_DUMP=<path>. */
    /* Every distinct configuration, not just the first: the interesting ones
     * are the shaders that draw nothing, and the first is the one that works.
     * CONKER_RC_DUMP=<prefix> writes <prefix>.0.hlsl, .1.hlsl and so on. */
    { static int dumped, checked;
      static const char *path;
      if (!checked) { path = getenv("CONKER_RC_DUMP"); checked = 1; }
      if (path && dumped < 16 && state->num_stages > 1) {
          static char src[65536];
          char name[512];
          int n = d3d8_combiners_generate_hlsl(state, src, sizeof(src));
          if (n > 0) {
              FILE *f;
              snprintf(name, sizeof(name), "%s.%d.hlsl", path, dumped);
              f = fopen(name, "wb");
              if (f) {
                  fwrite(src, 1, (size_t)n, f);
                  fclose(f);
                  fprintf(stderr, "[INFO RC] shader %d: %d bytes, %d stages -> %s\n",
                          dumped, n, state->num_stages, name);
              }
          } else {
              fprintf(stderr, "[INFO RC] generate_hlsl returned %d\n", n);
          }
          dumped++;
          fflush(stderr);
      } }

    /* Adjacent draws frequently share a program. Compare the entire key so
     * mutable constants, texture modes and all combiner fields remain covered,
     * while avoiding a serial byte-by-byte hash on every repeated draw. */
    if (g_last_shader_entry && g_last_shader_entry->in_use &&
        combiner_state_equal(&g_last_shader_entry->state, state)) {
        g_last_shader_entry->last_used_frame = g_frame_counter;
        return g_last_shader_entry->shader;
    }
     ...
     */
    {
        ...
        /* Keep the existing cache entry alive if compilation fails. */
...
/* Which combiner token, if any, is overriding the fixed-function pixel
 * path.  d3d8_combiners_prepare_draw() runs inside DrawPrimitiveUP, after
 * everything pgraph sets, so a non-zero token here means the texture-stage
 * states pgraph programmed are dead and this shader decides the pixel. */
DWORD d3d8_combiners_debug_token(void) { return g_ps_token; }
...
BOOL d3d8_combiners_prepare_draw(void)
{
    ...
    if (g_dirty) {
        ...
        g_combiner_state.bump_input[1] = 0;
        g_combiner_state.bump_input[2] = (rs[D3DRS_PSINPUTTEXTURE] >> 16) & 15u;
        g_combiner_state.bump_input[3] = (rs[D3DRS_PSINPUTTEXTURE] >> 20) & 15u;
        g_dirty = FALSE;
    }
    ...
    if (SUCCEEDED(hr)) {
        ...
        for (i = 0; i < NV2A_MAX_TEXTURES; ++i) {
            for (unsigned j = 0; j < 4u; ++j) {
                DWORD bits = d3d8_GetTextureStageStateValue(i, D3DTSS_BUMPENVMAT00 + j);
                memcpy(&cb->bump_matrix[i][j], &bits, sizeof(bits));
                cb->bump_luminance[i][j] = 0;
            }
            DWORD scale = d3d8_GetTextureStageStateValue(i, D3DTSS_BUMPENVLSCALE);
            DWORD offset = d3d8_GetTextureStageStateValue(i, D3DTSS_BUMPENVLOFFSET);
            memcpy(&cb->bump_luminance[i][0], &scale, sizeof(scale));
            memcpy(&cb->bump_luminance[i][1], &offset, sizeof(offset));
        }
