...
typedef enum NV2AOutputMapping {
    ...
    /* 5 is reserved in the hardware output mapping. */
...
typedef enum NV2ATextureMode {
    ...
    NV2A_TEXMODE_PROJECT2D = 4, /* Divide interpolated ST by interpolated Q */
    NV2A_TEXMODE_BUMPENV = 5, /* Signed BG perturbation from a prior texture */
    NV2A_TEXMODE_BUMPENV_LUM = 6,
} NV2ATextureMode;
 ...
 *   [18]   cd_blue_to_alpha - copy CD blue result to its destination alpha
 *   [19]   ab_blue_to_alpha - copy AB blue result to its destination alpha
 *
...
typedef struct NV2ACombinerState {
    ...
    unsigned bump_input[NV2A_MAX_TEXTURES];

    /* --- Flags --- */
    DWORD flags;     /* Dot mapping and other flags from token bits 24-31 */
    int mux_msb;     /* COMBINER_CONTROL bit 8: select with R0.a MSB, else LSB */
} NV2ACombinerState;
...
typedef struct NV2APSConstants {
    ...
    float bump_matrix[NV2A_MAX_TEXTURES][4]; /* 00,01,10,11 */
    float bump_luminance[NV2A_MAX_TEXTURES][4]; /* scale, offset, padding */
} NV2APSConstants;
