...
typedef enum D3DTEXTURESTAGESTATETYPE {
    ...
    D3DTSS_ALPHAARG0    = 27,
    D3DTSS_RESULTARG    = 28,
    /* Texture-coordinate transform.  PC D3D8 numbers this 24; on Xbox that
     * slot is D3DTSS_COLORKEYOP, so it takes the next free index here.  The
     * value is only ever passed between this layer and its callers. */
    D3DTSS_TEXTURETRANSFORMFLAGS = 29,
    /* Internal NV2A upper mip bound plus one; zero leaves the PC default. */
    D3DTSS_NV2A_MAXMIPLEVEL = 30,
} D3DTEXTURESTAGESTATETYPE;

/* Values for D3DTSS_TEXTURETRANSFORMFLAGS.  Projection is not implemented:
 * nothing needs it yet, and guessing at the semantics would be worse than
 * saying so. */
...
#define D3DFVF_TEXCOORDSIZE1(i) (3u << (16u + (i) * 2u))
#define D3DFVF_TEXCOORDSIZE4(i) (2u << (16u + (i) * 2u))
#define D3DFVF_NV2A_TEX4 (D3DFVF_TEXCOORDSIZE4(0) | D3DFVF_TEXCOORDSIZE4(1) | D3DFVF_TEXCOORDSIZE4(2) | D3DFVF_TEXCOORDSIZE4(3))
/* Host bridge: four UV pairs followed by a full-precision fog varying. */
#define D3DFVF_NV2A_FOG (D3DFVF_TEX5 | D3DFVF_TEXCOORDSIZE1(4))
...
/* Bounded live-render-target diagnostic.  It reports aggregate pixel state
 * without saving or importing a framebuffer/static-memory dump. */
extern int g_vtx_trace;
...
DWORD d3d8_GetDrawCount(void);
/*
 * A texture safe to sample when `target` is also the current render target.
 *
 * NV2A allows a draw to read and write the same surface; D3D11 does not, and
 * silently drops the render target if you try.  This returns a stand-in the
 * draw can sample instead, leaving `target` bound for writing.
 *
 * A host-renderer compatibility mechanism: it knows no addresses, no title and
 * no game state, and a D3D12 or Vulkan backend would express the same hazard
 * with barriers instead, replacing this body and nothing else.
 */
...
/* Runtime-only render-texture bridge used by the NV2A translator.  These
 * textures contain pixels produced during the current execution; no captured
 * framebuffer or static guest-memory image is involved. */
...
/* Borrow a GPU-generated color interpretation of an existing zeta surface. */
/* Explicit cube faces for NV2A uploads. The regular texture interface binds
 * the whole resource; its LockRect continues to address face zero. */
HRESULT d3d8_CreateCubeTexture(UINT edge, UINT levels, D3DFORMAT format,
                               IDirect3DTexture8 **texture);
HRESULT d3d8_LockTextureFace(IDirect3DTexture8 *texture, UINT face, UINT level,
                            D3DLOCKED_RECT *locked);
HRESULT d3d8_UnlockTextureFace(IDirect3DTexture8 *texture, UINT face, UINT level);
