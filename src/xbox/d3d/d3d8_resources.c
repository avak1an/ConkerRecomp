...
static ULONG __stdcall tex_Release(IDirect3DTexture8 *self)
{
    ...
    if (ref <= 0) {
        if (tex->rtv) ID3D11RenderTargetView_Release(tex->rtv);
...
HRESULT d3d8_LockTextureFace(IDirect3DTexture8 *self, UINT Face, UINT Level, D3DLOCKED_RECT *pLockedRect)
{
    D3D8Texture *tex = tex_from_iface(self);
    if (!pLockedRect || Level >= tex->levels || Face >= tex->faces) return E_INVALIDARG;
    if (tex->locked) return E_FAIL;

    size_t offset = Face * tex->face_bytes;
    UINT width = tex->width, height = tex->height;
    for (UINT level = 0; level < Level; ++level) {
        UINT rows = d3d8_format_is_compressed(tex->d3d8_format) ? (height + 3u) / 4u : height;
        offset += (size_t)d3d8_row_pitch(tex->d3d8_format, width) * rows;
        width = width > 1u ? width / 2u : 1u;
        height = height > 1u ? height / 2u : 1u;
    }
    pLockedRect->Pitch = (INT)d3d8_row_pitch(tex->d3d8_format, width);
    pLockedRect->pBits = tex->sys_mem + offset;
    tex->locked = TRUE;
    tex->locked_level = Level;
    tex->locked_face = Face;
    return S_OK;
}

HRESULT d3d8_UnlockTextureFace(IDirect3DTexture8 *self, UINT Face, UINT Level)
{
    D3D8Texture *tex = tex_from_iface(self);
    if (!tex->locked || Level != tex->locked_level || Face != tex->locked_face) return E_FAIL;

    tex->locked = FALSE;
    tex->dirty = TRUE;

    /* Upload only the level the caller locked. */
    ID3D11DeviceContext *ctx = d3d8_GetD3D11Context();
    if (ctx && tex->d3d11_texture) {
        UINT rows;
        BYTE *upload_data = tex->sys_mem + Face * tex->face_bytes;
        BYTE *unswizzled = NULL;
        UINT width = tex->width, height = tex->height;
        for (UINT level = 0; level < Level; ++level) {
            UINT level_rows = d3d8_format_is_compressed(tex->d3d8_format) ? (height + 3u) / 4u : height;
            upload_data += (size_t)d3d8_row_pitch(tex->d3d8_format, width) * level_rows;
            width = width > 1u ? width / 2u : 1u;
            height = height > 1u ? height / 2u : 1u;
        }
        UINT pitch = d3d8_row_pitch(tex->d3d8_format, width);
            ...
            d3d8_format_is_swizzled(tex->d3d8_format))
        {
            ...
            UINT linear_size = width * height * bpp;
            unswizzled = (BYTE *)malloc(linear_size);
            if (!unswizzled) return E_OUTOFMEMORY;
            xbox_unswizzle_rect(unswizzled, upload_data, width, height, bpp);
            upload_data = unswizzled;
        }

        ID3D11DeviceContext_UpdateSubresource(ctx,
            (ID3D11Resource *)tex->d3d11_texture,
            Level + Face * tex->levels, NULL, upload_data, pitch, pitch * rows);
        tex->dirty = FALSE;
...
static HRESULT __stdcall tex_LockRect(IDirect3DTexture8 *self, UINT level,
                                      D3DLOCKED_RECT *locked, const RECT *rect, DWORD flags)
{
    (void)rect; (void)flags;
    return d3d8_LockTextureFace(self, 0, level, locked);
}

static HRESULT __stdcall tex_UnlockRect(IDirect3DTexture8 *self, UINT level)
{
    return d3d8_UnlockTextureFace(self, 0, level);
}
...
static HRESULT create_texture(UINT Width, UINT Height, UINT Levels, DWORD Usage,
                              D3DFORMAT Format, UINT extra_bind, UINT faces,
                              IDirect3DTexture8 **ppTex)
{
    ...
    *ppTex = NULL;
    ...
    if (faces != 1u && (faces != 6u || Width != Height || Usage || extra_bind))
        return E_INVALIDARG;
    UINT max_levels = 1;
    ...
    /* Preserve the supplied chain, including sub-4x4 compressed levels. */
    UINT mip_width = Width, mip_height = Height;
    for (UINT level = 0; level < tex->levels; ++level) {
        UINT rows = d3d8_format_is_compressed(Format) ? (mip_height + 3u) / 4u : mip_height;
        data_size += (size_t)d3d8_row_pitch(Format, mip_width) * rows;
        mip_width = mip_width > 1u ? mip_width / 2u : 1u;
        mip_height = mip_height > 1u ? mip_height / 2u : 1u;
    }

    tex->face_bytes = data_size;
    tex->sys_mem = (BYTE *)calloc(faces, data_size);
    ...
    td.ArraySize = faces;
    td.MiscFlags = faces == 6u ? D3D11_RESOURCE_MISC_TEXTURECUBE : 0;
    td.Format = tex->dxgi_format;
    td.SampleDesc.Count = 1;
    td.Usage = D3D11_USAGE_DEFAULT;
    td.BindFlags = D3D11_BIND_SHADER_RESOURCE | extra_bind;
    ...
    if (faces == 6u) {
        ...
        srvd.TextureCube.MipLevels = tex->levels;
    } else {
    ...
    if (Usage & D3DUSAGE_RENDERTARGET) {
        hr = ID3D11Device_CreateRenderTargetView(d3d8_GetD3D11Device(),
            (ID3D11Resource *)tex->d3d11_texture, NULL, &tex->rtv);
        if (FAILED(hr)) {
...
HRESULT d3d8_CreateTextureImpl(UINT width, UINT height, UINT levels, DWORD usage,
                             D3DFORMAT format, IDirect3DTexture8 **texture)
{
    return create_texture(width, height, levels, usage, format, 0, 1, texture);
}

HRESULT d3d8_CreateCubeTexture(UINT edge, UINT levels, D3DFORMAT format,
                               IDirect3DTexture8 **texture)
{
    return create_texture(edge, edge, levels, 0, format, 0, 6, texture);
}

HRESULT d3d8_CreateShaderOutputTexture(UINT width, UINT height, IDirect3DTexture8 **texture)
{
    return create_texture(width, height, 1, 0, D3DFMT_A8B8G8R8,
                          D3D11_BIND_UNORDERED_ACCESS, 1, texture);
}
