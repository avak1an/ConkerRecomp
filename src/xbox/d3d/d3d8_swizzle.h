...
static inline int d3d8_format_is_swizzled(uint32_t fmt)
{
    /* Linear formats.
     *
     * Every LU_IMAGE_* and LC_IMAGE_* code, not just the five that happened to
     * be needed first.  A missing one is not a visible error: the texture is
     * unswizzled that was never swizzled, and comes out scrambled.  The same
     * list decides whether coordinates are in texels or 0..1, so a gap here
     * shows up twice. */
    if (fmt == 0x10 || /* LU_IMAGE_A1R5G5B5 */
        fmt == 0x11 || /* LU_IMAGE_R5G6B5   */
        fmt == 0x12 || /* LU_IMAGE_A8R8G8B8 */
        fmt == 0x13 || /* LU_IMAGE_Y8       */
        fmt == 0x17 || /* LU_IMAGE_G8B8     */
        fmt == 0x1B || /* LU_IMAGE_AY8      */
        fmt == 0x1C || /* LU_IMAGE_X1R5G5B5 */
        fmt == 0x1D || /* LU_IMAGE_A4R4G4B4 */
        fmt == 0x1E || /* LU_IMAGE_X8R8G8B8 */
        fmt == 0x1F || /* LU_IMAGE_A8       */
        fmt == 0x20 || /* LU_IMAGE_A8Y8     */
        fmt == 0x24 || /* LC_IMAGE_CR8YB8CB8YA8 (YUY2) */
        fmt == 0x25 || /* LC_IMAGE_YB8CR8YA8CB8 (UYVY) */
        fmt == 0x2E || fmt == 0x2F ||   /* LU_IMAGE_DEPTH_X8_Y24_*   */
        fmt == 0x30 || fmt == 0x31 ||   /* LU_IMAGE_DEPTH_Y16_*      */
        fmt == 0x35 || /* LU_IMAGE_Y16      */
        fmt == 0x3F || fmt == 0x40 || fmt == 0x41) /* linear RGBA permutations */
        return 0;
