...
static D3D11_SAMPLER_DESC g_sampler_desc[4];
...
static D3D11_BLEND d3d11_alpha_blend(D3D11_BLEND blend)
{
    /* D3D8 uses one RGBA factor. D3D11 requires its alpha component
     * separately and rejects *_COLOR in the alpha-factor fields. */
    switch (blend) {
...
static void update_blend_state(const DWORD *rs)
{
    ...
    bd.RenderTarget[0].SrcBlendAlpha = d3d11_alpha_blend(bd.RenderTarget[0].SrcBlend);
    bd.RenderTarget[0].DestBlendAlpha = d3d11_alpha_blend(bd.RenderTarget[0].DestBlend);
...
static void update_depth_stencil_state(const DWORD *rs)
{
    ...
    /* Compare the complete description. The old hash omitted all three
     * stencil operations and the write mask, reusing stale state objects. */
...
static D3D11_FILTER d3d8_to_d3d11_filter(DWORD mag, DWORD min, DWORD mip)
{
    ...
    return (D3D11_FILTER)D3D11_ENCODE_BASIC_FILTER(
        min_linear ? D3D11_FILTER_TYPE_LINEAR : D3D11_FILTER_TYPE_POINT,
        mag_linear ? D3D11_FILTER_TYPE_LINEAR : D3D11_FILTER_TYPE_POINT,
        mip_linear ? D3D11_FILTER_TYPE_LINEAR : D3D11_FILTER_TYPE_POINT,
...
void d3d8_states_apply_sampler(DWORD stage)
{
    ...
    if (sd.MaxAnisotropy > D3D11_REQ_MAXANISOTROPY) sd.MaxAnisotropy = D3D11_REQ_MAXANISOTROPY;
    memcpy(&sd.MipLODBias, &tss[D3DTSS_MIPMAPLODBIAS], sizeof(sd.MipLODBias));
    if (sd.MipLODBias < D3D11_MIP_LOD_BIAS_MIN) sd.MipLODBias = D3D11_MIP_LOD_BIAS_MIN;
    if (sd.MipLODBias > D3D11_MIP_LOD_BIAS_MAX) sd.MipLODBias = D3D11_MIP_LOD_BIAS_MAX;
    sd.ComparisonFunc = D3D11_COMPARISON_NEVER;
    sd.BorderColor[0] = ((tss[D3DTSS_BORDERCOLOR] >> 16) & 255u) / 255.0f;
    sd.BorderColor[1] = ((tss[D3DTSS_BORDERCOLOR] >> 8) & 255u) / 255.0f;
    sd.BorderColor[2] = (tss[D3DTSS_BORDERCOLOR] & 255u) / 255.0f;
    sd.BorderColor[3] = ((tss[D3DTSS_BORDERCOLOR] >> 24) & 255u) / 255.0f;
    sd.MaxLOD = tss[D3DTSS_MIPFILTER] == D3DTEXF_NONE ? 0.0f : D3D11_FLOAT32_MAX;
    if (tss[D3DTSS_MIPFILTER] != D3DTEXF_NONE) {
        sd.MinLOD = (float)tss[D3DTSS_MAXMIPLEVEL];
        if (tss[D3DTSS_NV2A_MAXMIPLEVEL])
            sd.MaxLOD = (float)(tss[D3DTSS_NV2A_MAXMIPLEVEL] - 1u);
        if (sd.MinLOD > sd.MaxLOD) sd.MinLOD = sd.MaxLOD;
    }

    if (g_sampler_states[stage] && !memcmp(&sd, &g_sampler_desc[stage], sizeof(sd))) {
