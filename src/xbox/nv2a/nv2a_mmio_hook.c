...
extern void nv2a_vblank_pump(void);   /* nv2a_core.c */
extern void recomp_dpc_drain(void);   /* kernel_thunks.c */
                           ...
                           uint32_t fault_xbox_va, int is_write)
{
    ...
    /* The display interrupt is raised from here because this is the guest
     * thread and the access it faulted on has already been emulated: the
     * service routine can run without disturbing the instruction being
     * completed, and its own register accesses simply fault again the same
     * way.  A timer thread would be the obvious place and is the wrong one --
     * the lifted register file is per-thread, so the routine would run
     * against an uninitialised guest stack. */
    nv2a_vblank_pump();

    /* Deferred procedure calls run after the interrupt that raised them, on
     * the guest thread, which is the only place the lifted register file is
     * coherent.  Draining an empty queue is a load and a branch, so this is
     * free until something actually queues one. */
    recomp_dpc_drain();
