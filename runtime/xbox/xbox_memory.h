...
/** Advance the bump allocator past a fixed restored-memory region. */
void xbox_heap_reserve_through(uint32_t end_va);
void xbox_heap_reserve_range(uint32_t start_va, uint32_t end_va);

/**
 * Free a heap allocation.
 * Returns the block to a coalescing free list, so the address space can be
 * handed out again.  A block at the top of the heap rolls the cursor back.
 */
void xbox_heap_free(uint32_t xbox_va);

/** Log heap occupancy: cursor, live bytes, free bytes, and bytes recycled. */
