...
class Disassembler:
    ...
    def __init__(self, xbe_path: str,
                 ...
                 icall_targets: Optional[list] = None):
        ...
        # Indirect-branch targets observed at run time. Kept apart from the
        # seed list: a seed is asserted to be a function start, while an
        # observed target may well be a mid-function entry, and the detector
        # decides which once the extents are final.
        self.icall_targets = icall_targets or []
    ...
    def run(self) -> bool:
        ...
        if self.seed_functions:
            self.func_detector.add_configured_functions(self.seed_functions)
            if self.verbose:
                print(f"  Seeded {len(self.seed_functions)} function addresses")

        if self.icall_targets:
            self.func_detector.add_observed_targets(self.icall_targets)
            if self.verbose:
                print(f"  {len(self.icall_targets)} observed icall targets "
                      f"to resolve after detection")
